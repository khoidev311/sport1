...
const HomeLeagueItem = ({ league, isActive, onSelectLeague }: HomeLeagueItemProps) => {
  ...
  return (
    <div
      ...
      className={twMerge(
        "flex group relative px-4 group hover:bg-gray-100 py-2 w-full justify-start h-full items-center xs:gap-x-2 xl:gap-x-4 before:absolute before:left-0 before:top-1/2 before:h-9 before:w-1 before:-translate-y-1/2 before:rounded-sm before:from-blue-500 before:to-purple-500 before:bg-gradient-to-b  from-blue-500 to-purple-500 before:hidden hover:before:block",
        isActive && "before:block",
      )}
    >
      <img className="w-6 h-6 object-scale-down" alt={league.name} src={league.logo} />
      <div className="text-sm h-6 flex items-center w-fit line-clamp-1  font-semibold group-hover:text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">
        {league.name}
      </div>
    </div>
  );
